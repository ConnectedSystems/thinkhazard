...
class Harvester(BaseProcessor):
    ...
    def fetch(self, category, params={}, order_by='title'):
        ...
        if response.status != 200:
            raise Exception('Geonode returned status {}: {}'.
    ...
    def hazardtype_from_geonode(self, geonode_name):
        for mnemonic, type_settings in \
                self.settings['hazard_types'].items():
        ...
        with open(region_admindiv_csv_path, 'rb') as csvfile:
            ...
            for row in rows:
                try:
                    ...
                    transaction.abort()
                    logger.error('Linking region {} & admin div {} failed'
                                 .format(row[1], row[2]))
                    logger.error(traceback.format_exc())
    ...
    def create_region_admindiv_association(self, row):
        ...
        if region is None:
            logger.warning('Region {} id {} does not exist in GeoNode'
                           .format(row[0], row[1]))
            return False
        ...
        if admindiv is None:
            logger.warning('AdminUnit code {} is not in our GAUL dataset'
                           .format(row[2]))
            return False
    ...
    def harvest_regions(self):
        ...
        for region in regions:
            try:
                ...
                transaction.abort()
                logger.error('Region {} raises an exception :\n{}'
    ...
    def harvest_documents(self):
        ...
        for doc in documents:
            try:
                ...
                transaction.abort()
                logger.error('Document {} raises an exception :\n{}'
    ...
    def harvest_document(self, object):
        ...
        for region in regions:
            for type in hazardtypes:
                ...
                logger.info('  Linked with Region {} for HazardType {}'
    ...
    def harvest_layers(self, hazard_type=None):
        ...
        for layer in layers:
            try:
                ...
                transaction.abort()
                logger.error('Layer {} raises an exception :\n{}'
